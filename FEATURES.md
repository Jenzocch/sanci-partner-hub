...
### Kalkulator Keranjang desktop UI（2026-09-07）
...
| P2-39 | 訂單建立時自動複製 Package 內容 | `UNVERIFIED` | `createCustomerAndOrder`（cabang 端 Server Action）成功建單且有選 Package 時，把 `partner_package_items` 逐行複製成 `order_items`（名稱/代碼從 `sanci_products` 現查現寫入，不信任 client 快照）。**Best-effort**：複製失敗不影響、不回滾已建立的訂單（跟 invoice 附件同一套邏輯），但失敗會回報成 UI 警告橫幅，不會靜默吞掉（LESSONS #10）。每行用確定性 `client_request_id`（`{訂單請求碼}:item:{partner_package_items.id}`）防止重試造成重複。**2026-09-01 改鍵**：原本用 `{product_id}`，唯一性靠 `unique (package_id, product_id)`（0012）撐著——那條 constraint 是產品決策不是技術承諾，一旦放寬（例如套裝要支援同款雙色），第二列會被 `ON CONFLICT DO NOTHING` 靜默吞掉。改用套裝列自己的 primary key，唯一性憑構造（LESSONS #50）；已在真 Postgres 證明舊鍵存 1 列、新鍵存 2 列、重試仍 2 列（`test-harness/130_behavior_package_item_key.sql`） |
...
### 名稱/代碼欄位全面加「用途說明」（2026-08-24，owner：「設定名稱 id 要有說明」）

開通流程的每個名稱/代碼/ID 欄位加上 1～2 句「誰會看到、用在哪裡」的說明
（12 檔、三語）：Partner 名稱（admin 全畫面＋分店端首頁識別）、Partner 代碼
（訂單編號開頭＋登入 ID 組成；**啟用後鎖定**——經 `updatePartner` 只在 DRAFT
收 code 的 server 端證據核實）、分行名稱/代碼（訂單編號＋客戶編號組成）、
帳號 Name（純列表標籤，非登入用）、員工姓名（Sales/PIC 下拉＋**只印在 SO**
——查列印頁證實 DO/Invoice 不印，說明只寫 SO）、員工代碼（客戶編號的 AS 段，
...
- `sanci_products.size` — 單一 text 欄位、可為空、**無 CHECK**：實際資料含
  `180*200*30`、`(1200-1550)*1200`（可延伸餐桌）、`60*36*9/7`（雙高度枕），
  強制數字結構會拒絕今天就合法的資料。與 `shipping_address`(0014)、
  `customer_po`(0020) 同一類。
- 不碰 RLS（新欄位自動沿用 0022 的三條 policy——含匿名公開頁可讀，尺寸本
  來就是要給客人看的規格）、不重定義 `fn_audit_row`（`to_jsonb` 自動涵蓋，
  與 0014/0020 相同先例）、無新 trigger/constraint。
- 顯示：分店產品詳情頁與公開產品頁各加一列「Ukuran」，**排在價格上方**
  （店員先回答「放不放得下」再談價錢）。i18n 三語。
- 資料匯入分開交付（105 個既有產品），SQL 只寫 size、**完全不碰價格**，
  且只補空值不覆蓋（LESSONS #39）。
...
- [x] Migration 0024/0025/0026/0027 production 驗證（audit 2026-09-08 P2-2：這四個檔案先前完全沒有 FEATURES.md 追蹤紀錄，掉進空隙。owner 貼回唯讀檢查 SQL 結果：`sanci_products.size`、`product_colors` 表、`has_color_options`、`customer_total_amount`/`expedition`、`fn_guard_customer_payment`、`customer_settled_on` 全部 `ya`；兩條護欄基準同時核對——`sanci_products` 仍 0 個價格欄位、`fn_audit_row` 已含 `PRODUCT_COLOR` 映射）
- [x] Migration 0028（`idx_audit_logs_scope_time`/`idx_audit_logs_entity`/`idx_customers_created_via`/`idx_staff_assignments_branch` 四個索引）已在 production 執行並驗證（2026-09-08，owner 貼回結果：四個 `*_INDEX` 皆為 1；當時列數 `audit_logs`=1613、`customers`=44、`partner_staff_assignments`=2，記錄供日後比對索引是否仍對應查詢型態）
