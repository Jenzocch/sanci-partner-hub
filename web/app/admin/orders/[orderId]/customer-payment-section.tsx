...
import { getCustomerPaymentAdmin, setCustomerPaymentAdmin } from "../../actions-orders";

/**
 * Kartu "Pembayaran Pelanggan" (Fitur D, migrasi 0026) — sisi admin. Mirror
 * PERSIS kembarannya di app/cabang/pesanan/[orderId]/customer-payment-section.tsx
 * (SENGAJA komponen terpisah, bukan satu komponen dibagi — dua Server Action
 * berbeda modul dengan lapisan identitas berbeda, pola sama dengan
 * OfferSection/OrderOfferForm). Admin di sini SELALU boleh mengubah (tidak
 * ada `canManage` — RLS admin-all yang menegakkan, LESSONS #5).
 */
...
export default function CustomerPaymentSection({
  ...
  async function loadFresh() {
    ...
    try {
      const res = await getCustomerPaymentAdmin(orderId);
      if (seq !== loadSeq.current) return;
      if (res.status !== "ok") {
        setLoad({ status: "error", message: res.status === "unavailable" ? m.admin.customerPaymentFeatureOff : undefined });
        return;
      }
  ...
  const liveRemaining = liveTotal != null && livePaid != null ? liveTotal - livePaid : null;
    ...
    <div className="card">
      ...
      <div className="btnrow-inline">
        <button className="btn primary" onClick={openModal}>
          {m.admin.customerPaymentEditBtn}
        </button>
      </div>
      ...
      {open && (
        <div className="overlay" onClick={(e) => e.target === e.currentTarget && closeModal()}>
          <div className="modal" role="dialog" aria-modal="true">
            <h2>{m.admin.customerPaymentModalTitle}</h2>
            ...
            {load.status === "error" && (
              <>
                <div className="banner bad">{load.message ?? m.admin.customerPaymentLoadFailed}</div>
                <div className="btnrow">
                  <button type="button" className="btn" onClick={closeModal}>
                    {m.common.close}
                  </button>
              ...
              <form onSubmit={onSubmit}>
                ...
                <div className="btnrow">
                  ...
                  <button type="submit" className="btn primary" disabled={submitting}>
                    {submitting ? m.common.saving : m.admin.customerPaymentSaveBtn}
                  </button>
                </div>
              </form>
            )}
