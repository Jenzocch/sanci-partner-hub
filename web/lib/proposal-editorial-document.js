...
/**
 * `store` — blok "Toko" di sampul, HANYA diisi sisi CABANG (owner 2026-09-07:
 * sampul tetap SANCI, ditambah kontak toko partner supaya pelanggan tahu ia
 * membeli lewat siapa). Sisi admin tidak mengisi (proposal SANCI sendiri).
 * Bentuk: { name, branchName, phone, logoUrl } — semuanya boleh null;
 * tanpa logo → hanya nama teks (keputusan owner).
 *
 * JSDoc di bawah adalah SATU-SATUNYA sumber tipe props berkas .js ini untuk
 * TypeScript (allowJs): tanpa ini `store = null` disimpulkan bertipe `null`
 * dan halaman .tsx tidak bisa mengisinya (tsc TS2322, terukur 2026-09-07).
 *
 * @param {{
