...
const id = {
  ...
  calcExtraFeeLabelField: "Biaya tambahan (opsional)",
  calcExtraFeeLabelPlaceholder: "cth. Ongkir, Pemasangan",
  calcExtraFeeAmountField: "Nominal (Rp)",
  calcExtraFeeHint:
    "Ditambahkan paling akhir, tidak kena diskon. Kosongkan kalau tidak ada — baris ini tidak dicetak di Proposal.",
  calcExtraFeeNotInOffer:
    "Biaya tambahan \"{label}\" {amount} TIDAK ikut ke Penawaran pesanan (tidak ada tempatnya di rantai diskon) — " +
    "tambahkan manual kalau perlu.",
...
const en = {
  ...
  calcExtraFeeLabelField: "Additional charge (optional)",
  calcExtraFeeLabelPlaceholder: "e.g. Shipping, Installation",
  calcExtraFeeAmountField: "Amount (Rp)",
  calcExtraFeeHint:
    "Added last, never discounted. Leave empty if none — the line is then not printed on the Proposal.",
  calcExtraFeeNotInOffer:
    "The additional charge \"{label}\" {amount} is NOT carried into the order's Offer (the discount chain has no slot " +
    "for it) — add it manually if needed.",
...
const zh = {
  ...
  calcExtraFeeLabelField: "额外费用(可选)",
  calcExtraFeeLabelPlaceholder: "例如 运费、安装",
  calcExtraFeeAmountField: "金额(Rp)",
  calcExtraFeeHint: "最后加上,不参与折扣。没有就留空——留空时Proposal不会印这一行。",
  calcExtraFeeNotInOffer: "额外费用「{label}」{amount} 不会带入订单的报价(折扣链没有它的位置),需要的话请手动加上。",
