...
- 每一列一筆訂單，用 **A 欄「訂單編號」** 認人。已經在表上的訂單會**就地更新**，
  新訂單**加在最下面**。
- 欄位（**A 到 N 共 14 欄** —— 2026-08-20 遷移 0014 之後從原本的 A..K 擴充，
  詳見下方「⚠️ 欄位範圍變更」）：
...
## ⚠️ 欄位範圍變更（2026-08-20，migrasi 0014）

這份文件原本承諾「只會寫 A 到 K，絕不碰 L 欄以後」——這個承諾**在這次更新被打破了**，
因為新增的三個欄位（Uang Muka/DP、Kondisi Pembayaran、Alamat Kirim）讓表格本身從
11 欄變成 14 欄，原本的「舊 L 欄」在新版裡變成「新 O 欄」。這裡不假裝什麼都沒變，
直接講清楚後果：

- **如果你在舊版的 L 欄自己寫了備註**，那份資料**還在原本的儲存格裡**，
  但同步程式現在會嘗試把 J/K/L 欄寫進 Uang Muka/Kondisi Pembayaran/Alamat Kirim ——
  如果你的分頁還是舊格式，程式會**拒絕同步那個分頁**並在執行紀錄留言（不會盲目
  覆蓋你的備註），照下面的指示處理。
- **偵測方式**：同步程式會比對分頁第一列（標題列）是不是跟上面的 14 欄標題完全一致。
  不一致（例如還是舊的 11 欄格式）就直接**跳過那個分頁**，其他分頁不受影響。
...
- ❌ **永遠不會刪除任何一列**。訂單取消了，只是 F 欄變成「Dibatalkan」，
  那一列還在。
- ❌ **不會碰 O 欄以後的任何東西**（0014 之前是「L 欄以後」——見上方「欄位範圍
  變更」）。你想在右邊自己加備註欄、加公式、加顏色，儘管加——同步程式只會寫
  A 到 N，不會蓋掉你寫的東西。
...
## 八、關於 I／J／K 欄「Penawaran SANCI／Uang Muka／Kondisi Pembayaran」

這三欄都是後台**訂單詳細頁**「Penawaran SANCI」卡片裡填的資料，性質完全相同：
**人在後台手動打進去的數字/文字，資料庫不對它們做任何計算**（沒有折扣公式，沒有
自動加總——這條規矩從 `0009` 就定下來，`0014` 也沒有打破它，細節見
`web/lib/i18n/GLOSSARY.md`）。

- 它們**只有 SANCI 看得到**，除非後台「Hak Akses」分頁裡個別合作商被打開
  「可以查看／填寫 Penawaran SANCI」——這是資料庫層級（RLS）擋掉的權限，
  不是只把畫面藏起來而已。
...
| **某個分頁的執行紀錄寫「Format lama terdeteksi」** | 那個分頁還是舊版 11 欄格式（`0014` 之前建立的），跟現在 14 欄的標題對不上 | 照訊息指示：把那個分頁改名或搬走存檔，再重新 Sync，系統會用新格式建一個同名新分頁 |
