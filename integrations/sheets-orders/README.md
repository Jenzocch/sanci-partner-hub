...
| **Dokumen** | 一列一個「單據 × 品項」，**16 欄**——查「DO-002 這張出貨單到底出了什麼」 | 每次整頁重寫 |

> ⚠️ **Item Pesanan、Pelanggan、Warna、Dokumen 四個分頁每次同步都會整頁重寫**，
> 所以**不要在這四個分頁裡寫任何備註**，會被蓋掉。要寫備註請寫在合作商分頁的
> **AP 欄以後**（那裡永遠不會被碰）。

### Dokumen 分頁欄位（16 欄）

合作商分頁和 Item Pesanan 分頁裡的 `No. DO` / `No. Invoice` 是**每筆訂單的彙
總**——多張 DO 用 `+` 串起來、日期取最後一張、數量跨所有 DO 加總。那能回答
「這筆訂單出貨了沒」，但**回答不了「DO-002 那張單子裡面到底有哪幾項、各幾件」**。
分批出貨（0016 從一開始就設計支援的情境）差別就在這裡。

這個分頁把**每一張單據當成它自己**存一份，讓不會還原 pg_dump 的人也能把任何
一張出貨單或發票完整讀回來。

| 欄 | 內容 |
|---|---|
| A | 單據類型（SO / Surat Jalan (DO) / Invoice） |
| B | 單據號碼 |
| C | 單據日期 |
| D | 系統訂單編號 |
| E | 合作商 |
...
**L 欄和 M 欄故意並排**：兩者的差就是分批出貨，放在同一列讓人不用算就看得出來。
例如訂單有 5 張床，DO-001 出 3 張、DO-002 出 2 張，就會看到兩列 `L=3 / M=5` 和
`L=2 / M=5`。

排序是**日期 → 單據號碼**，跟辦公室紙本單據的存放順序一致。
