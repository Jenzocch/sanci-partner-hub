...
- 每一列一筆訂單，用 **A 欄「訂單編號」** 認人。已經在表上的訂單會**就地更新**，
  新訂單**加在最下面**。
- 欄位（**A 到 T 共 20 欄** —— 2026-08-27 遷移 0020 又在 B 欄插入一欄，把
  2026-08-20 遷移 0015 那次擴充出來的 A..S 19 欄全部往右推一欄，詳見下方
  「⚠️ 欄位範圍變更」）：

  | 欄 | 標題 | 說明 |
  |---|---|---|
  | A | Nomor Pesanan | 訂單編號（比對用的鑰匙，不要手動改） |
  | B | No. PO Pelanggan | 客戶自己開的採購單（PO）編號，自由格式——沒有就空白，需要 `0020` 已執行 |
  | C | Cabang | 分店 |
  | D | Pelanggan | 客戶姓名 |
  | E | Telepon | 電話 |
  | F | Package | 套裝名稱 |
  | G | Status | 已登記 Terdaftar／已取消 Dibatalkan |
  | H | Jalur Pesanan | 交付方式：直接送貨／到店選購 |
  | I | Belanja Toko (IDR) | 客戶在合作商店裡消費的金額（分店回報） |
  | J | Penawaran SANCI (IDR) | **SANCI 方案金額**（你在後台填的基礎金額，未套用折扣/加成/現金折讓之前的數字） |
  | K | Uang Muka / DP (IDR) | 訂金（後台可填，需要 `0014` 已執行） |
  | L | Kondisi Pembayaran | 付款條件，自由文字（例：Full payment、DP 50%） |
  | M | Alamat Kirim | 這筆訂單的收貨地址（分店可填，需要 `0014` 已執行） |
  | N | Diskon | 折扣鏈，格式「8+10」表示先減 8% 再減 10%（連乘，不是相加）——需要 `0015` 已執行 |
  | O | Markup (%) | 折扣之後再加成的百分比——需要 `0015` 已執行 |
  | P | Potongan Tunai (IDR) | 最後扣除的現金折讓（湊整數/現金優惠用）——需要 `0015` 已執行 |
  | Q | Harga Akhir (IDR) | **資料庫算出來的最終應付金額**（基礎金額 → 折扣鏈 → 加成 → 減現金折讓）——需要 `0015` 已執行 |
  | R | Sisa (IDR) | Harga Akhir 減 Uang Muka（純顯示用的算式，不是資料庫欄位）——需要 `0015` 已執行 |
  | S | Dibuat | 建立時間 |
  | T | Diubah | 最後修改時間 |
...
## ⚠️ 欄位範圍變更（2026-08-20 migrasi 0014 與 0015 各一次；2026-08-27 migrasi 0020 又一次）

這份文件原本承諾「只會寫 A 到 K，絕不碰 L 欄以後」——這個承諾在 `0014` 就已經被打破
成「只會寫 A 到 N」，`0015` 又新增五欄變成「只會寫 A 到 S」，這次 `0020` **在 B 欄
插入**一欄「No. PO Pelanggan」，把原本 B 到 S 的每一欄都往右推一欄，變成「只會寫
A 到 T，絕不碰 U 欄以後」。這裡不假裝什麼都沒變，直接講清楚後果：

- **這次跟前兩次不一樣的地方**：0014 和 0015 都是把新欄**加在最後面**（Dibuat/
  Diubah 之前），原本 B..K 那些欄位的字母完全沒有變過。這次 0020 是**插在最前面**
  （B 欄，緊接在 Nomor Pesanan 右邊），所以**從 C 欄開始，每一欄的字母都跟舊版
  差一格**（舊版的 Cabang 是 B 欄，新版變成 C 欄；以此類推到舊版的 Diubah S 欄，
  新版變成 T 欄）。如果你在舊分頁上用欄位字母寫過公式、篩選條件或設過條件式格式，
  那些用字母指定欄位的設定在**新建的分頁**裡會對不上——舊分頁本身不會被動到
  （見下一點），但换到新分頁後你需要照新的字母表重新設定。
- **如果你在舊版（0020 之前）的分頁上自己寫了備註或公式**，那份資料**還在原本的
  儲存格裡，不會被動到**——同步程式偵測到你的分頁還是舊格式（不管是 0014 之前的
  11 欄、0014～0015 之間的 14 欄，還是 0015～0020 之間的 19 欄）就直接**拒絕同步
  那個分頁**並在執行紀錄留言（不會盲目覆蓋你的備註），照下面的指示處理。
- **偵測方式**：同步程式會比對分頁第一列（標題列）是不是跟上面的 20 欄標題**逐格
  完全一致**（包含順序）。不一致（例如還是舊的 11／14／19 欄格式）就直接**跳過
  那個分頁**，其他分頁不受影響。
- **怎麼處理舊分頁**：把那個分頁改名（例如加上「(舊)」），或整個搬到另一份試算表
  存檔，然後重新跑一次 Sync——系統會用新格式（含 B 欄新的 No. PO Pelanggan）重新
  建立一個同名的新分頁。
- **0014→0015 中間狀態（只跑了 0014 沒跑 0015）優雅降級**：J/K/L/M 欄照常，
  N/O/P/Q/R 五欄各自獨立偵測缺欄位並留空，不會拖累其他欄位（跟 0013→0014 中間
  狀態的降級邏輯一樣）。
- **0015→0020 中間狀態（還沒跑 0020）優雅降級**：B 欄（No. PO Pelanggan）留空，
  其餘 C..T 欄照常——同步不會因為 customer_po 這一欄讀不到就整批失敗。

### 升級步驟（已經照第四章裝過舊版 Code.gs 的人看這裡）

**只需要重新貼 Code.gs，不用動 Supabase（migration 0020 你已經跑過了），試算表
本身也不用手動改任何東西**——同步程式自己會處理舊分頁。步驟：

1. 打開你的試算表 → **擴充功能** → **Apps Script**。
2. 打開 `Code.gs`，**全選、刪除**，把這份資料夾裡**新版** `Code.gs` 的內容全選、
   複製、貼上進去。
3. 按 💾 儲存（或 Ctrl+S / ⌘+S）。**不需要重新填第五步那四個 Script Properties**
   （`SUPABASE_URL` 等），它們沒有變。也**不需要重新跑 `setupTrigger`**，既有的
   15 分鐘排程照常生效，下一輪自動跑到就會套用新版程式。
4. 想立刻看到效果，回到試算表 → 選單 **SANCI Sync** → **Sync sekarang**。
5. 看執行紀錄（**執行項目** → 最新一次）：
   - 每個既有分頁大機率會出現
     `TAB GAGAL "合作商名稱": Error: Format lama terdeteksi ...`——**這是預期
     行為，不是壞掉**：因為 B 欄插入把舊分頁的 19 欄標題跟新版 20 欄標題對不
     上，程式照設計拒絕動那個分頁，你原本的資料完全沒被碰。
   - 每個被拒絕的分頁，照第一節「⚠️ 欄位範圍變更」的「怎麼處理舊分頁」指示：
     把該分頁改名（例如「合作商 A (舊 2026-08-27 前)」）或搬到別的試算表存檔。
6. 改完名字後，再點一次 **SANCI Sync → Sync sekarang**——這次每個合作商會拿到
   一個全新、20 欄格式（含 B 欄 No. PO Pelanggan）的同名分頁，從這一輪抓到的
   訂單開始寫入；改名保留下來的舊分頁還在，資料原封不動，只是不會再被同步
   更新了。
7. 如果你想把舊分頁的歷史資料搬進新分頁（例如你在舊分頁 T 欄以後加過的自訂
   備註），這是**手動**的事——同步程式從不做這件事（見第二章）。

> 不想現在升級也可以：舊版 Code.gs 不會自己壞掉，繼續同步 A..S 19 欄，
> 只是新訂單的 PO 編號不會出現在試算表上（後台跟 Invoice 上還是看得到）。

## 二、這個工具**不做**什麼（很重要，先看完再用）

- ❌ **不會反向同步**。你在試算表上改任何格子，**系統裡不會跟著變**。
  試算表是「看的」，不是「填的」。真正要改資料，還是要回後台改。
- ❌ **永遠不會刪除任何一列**。訂單取消了，只是 G 欄（Status）變成「Dibatalkan」，
  那一列還在。
- ❌ **不會碰 U 欄以後的任何東西**（0014 之前是「L 欄以後」，0014→0015 之間是
  「O 欄以後」，0015→0020 之間是「T 欄以後」——見上方「欄位範圍變更」）。你想在
  右邊自己加備註欄、加公式、加顏色，儘管加——同步程式只會寫 A 到 T，不會蓋掉你
  寫的東西。
...
## 八、關於 J..R 欄「Penawaran SANCI」相關欄位
...
| **某個分頁的執行紀錄寫「Format lama terdeteksi」** | 那個分頁還是舊版格式（11／14／19 欄，`0014`/`0015`/`0020` 之前建立的），跟現在 20 欄的標題對不上 | 照訊息指示：把那個分頁改名或搬走存檔，再重新 Sync，系統會用新格式建一個同名新分頁（見「升級步驟」一節） |
| 某個合作商的分頁沒更新，其他都正常 | 那個分頁可能被保護／鎖定，或名稱有特殊字元 | 看執行紀錄裡的 `TAB GAGAL "合作商名稱": ...`，照後面的訊息處理 |
| 同一筆訂單出現兩列 | A 欄的訂單編號被人手動改過（改了就對不上，會被當成新訂單） | 把多出來的那一列刪掉，並且**不要再手動編輯 A 到 T 欄** |
