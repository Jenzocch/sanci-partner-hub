...
「訂單清單」升級成「資料存檔」**——除了每個合作商一個分頁，另外多了兩個全系統
共用的分頁。
...
粗體 = 2026-08-31 新增。V..AA 六欄需要 `0016`（文件功能）。
...
## 二、⚠️ 這次升級要做什麼（2026-08-31，第四次欄位變更）

這份文件承諾的「絕不碰某欄以後」已經被打破四次：`0014`（K→N）、`0015`（N→S）、
`0020`（S→T，而且是插在 B 欄）、以及這次（T→AE）。不假裝沒變，直接講後果。

- **這次是加在後面，不是插在中間。** A..U 這些欄位的字母**完全沒有變**（U 欄
  以前就是 Sisa，現在還是 Sisa）。新的 11 欄接在 Sisa 後面，原本的 Dibuat/
  Diubah 從 S/T 移到 **AD/AE**。所以只有引用到 S/T 兩欄的公式需要改。
- **你的手寫備註不會被動到。** 同步程式偵測到分頁還是舊格式（11／14／19／20 欄）
  就**直接拒絕同步那個分頁**並在執行紀錄留言，不會盲目覆蓋。
- **偵測方式**：比對第一列標題是不是跟 31 欄標題**逐格完全一致**。不一致就跳過
  那個分頁，其他分頁不受影響。
- **手寫備註的新起點是 AF 欄**（原本是 U）。
- **優雅降級**：`0016` 沒跑 → V..AA 六欄留空、K 欄留空；`0023` 沒跑 → AB 欄留空；
  `0017`–`0019` 沒跑 → E 欄留空。每一欄各自獨立偵測，**不會因為一個欄位讀不到
  就整批失敗**。
...
5. 看執行紀錄（**執行項目** → 最新一次）：
   - 每個既有分頁**大機率會出現** `TAB GAGAL "合作商名稱": Error: Format lama
     terdeteksi ...`——**這是預期行為，不是壞掉**。你原本的資料完全沒被碰。
   - **`Item Pesanan` 和 `Pelanggan` 兩個新分頁會直接建立好**，不受影響。
6. 把每個被拒絕的分頁**改名**（例如「合作商 A (舊 2026-08-31 前)」）或搬到別的
   試算表存檔。
7. 再點一次 **SANCI Sync → Sync sekarang**——這次每個合作商會拿到一個全新的
   31 欄分頁。改名保留的舊分頁還在，資料原封不動，只是不會再被更新。
8. 舊分頁的手寫備註要不要搬到新分頁，是**手動**的事——同步程式從不做這件事。

> 不想現在升級也可以：舊版 Code.gs 不會自己壞掉，繼續同步 20 欄，只是不會有
> 出貨狀態、SO/DO/Invoice 號碼，也不會有品項和客戶兩個分頁。
...
- ❌ **不會碰合作商分頁 AF 欄以後的任何東西**（歷次是 L → O → T → U → AF，見上方
  「欄位範圍變更」）。你想在右邊自己加備註欄、加公式、加顏色，儘管加——同步程式
  只會寫 A 到 AE。
- ⚠️ **但 `Item Pesanan` 和 `Pelanggan` 兩個分頁例外**：它們每次同步都整頁重寫，
  在裡面寫什麼都會被蓋掉。備註請寫在合作商分頁的 AF 欄以後。
...
| **某個分頁的執行紀錄寫「Format lama terdeteksi」** | 那個分頁還是舊版格式（11／14／19／20 欄），跟現在 31 欄的標題對不上 | 照訊息指示：把那個分頁改名或搬走存檔，再重新 Sync，系統會用新格式建一個同名新分頁（見「升級步驟」一節） |
| 某個合作商的分頁沒更新，其他都正常 | 那個分頁可能被保護／鎖定，或名稱有特殊字元 | 看執行紀錄裡的 `TAB GAGAL "合作商名稱": ...`，照後面的訊息處理 |
| 同一筆訂單出現兩列 | A 欄的訂單編號被人手動改過（改了就對不上，會被當成新訂單） | 把多出來的那一列刪掉，並且**不要再手動編輯 A 到 AE 欄** |
