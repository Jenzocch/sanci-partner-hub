...
> `0001 → 0003 → 0004 → 0005 → 0006 → 0007 → 0008 → 0009 → 0010 → 0011 → 0012 → 0013 → 0014 → 0015 → 0016 → 0017 → 0018 → 0019 → 0020 → 0021 → 0023`.**
...
| 0021 | tidak ada — 0021 adalah berkas terakhir dalam rantai saat ini, jadi menjalankan ulangnya sendirian tidak kehilangan apa pun; idempotensi diukur (3× di atas rantai penuh, nol diff `pg_dump -s` tersaring, ke-39 angka verifikasi identik di run ke-3). Lebih penting: 0021 kini **pemulih `fn_audit_row`** — versinya memuat SELURUH perilaku 0004+0005+0008+0009+0010+0012+0013+0014+0016+0018 plus `PRODUCT_PRICE` (salinan utuh 0018 + satu baris pemetaan, di-diff langsung), dan TERUKUR: setelah 0001 dijalankan ulang di atas rantai penuh (prosrc kehilangan `PRODUCT_PRICE` DAN `CUSTOMER_SOURCE`), SATU kali re-run 0021 mengembalikan keduanya (plus `ORDER_DOCUMENT`) sekaligus. Guard §0-nya sengaja memeriksa KEBERADAAN objek 0018/0019/0020 (bukan versi aktif `fn_audit_row`) supaya jalur pemulihan ini tidak terkunci oleh guard-nya sendiri — lihat komentar §0 di berkasnya. Yang TIDAK dipulihkan 0021: semua yang bukan `fn_audit_row` (policy `c_partner_read`/`s_partner_read` 0007, `fn_check_order_refs` 0011, constraint `dp_le_*` 0015, `fn_set_customer_code` 0019 — pemulihnya tetap berkas masing-masing seperti baris-baris di atas). |
| 0023 | tidak ada — 0023 **TIDAK mendefinisikan ulang `fn_audit_row`** (garis merah penugasannya) dan tidak mendefinisikan ulang satu pun fungsi/policy/constraint milik berkas lain, jadi menjalankan ulangnya tidak pernah mencabut hasil kerja berkas mana pun. Sebaliknya juga aman: menjalankan ulang berkas LAMA tidak menghapus apa pun milik 0023 (kolom, index unik, tabel, kedua RPC, dan `trg_order_customer_link` semuanya bernama SENDIRI dan tidak disebut berkas lain). Idempotensi DIUKUR: 3× dijalankan ulang di atas rantai penuh `0001→…→0021→0023`, nol diff `pg_dump -s` (penyaring `\restrict` LESSONS #33), ke-35 angka verifikasi identik. Satu hal yang PERLU diperhatikan kalau `ADD COLUMN` pernah gagal separuh jalan: blok jaring pengaman §1 mengisi token yang NULL dengan `alter table … disable trigger user` di sekelilingnya — itu disengaja (tanpa itu, pesanan berstatus CANCELLED ditolak `fn_guard_order_status_flow` 0005 dan migrasinya gagal di tengah), dan seluruh berkas berjalan dalam satu transaksi sehingga kegagalan mengembalikan trigger bersama sisanya. |
