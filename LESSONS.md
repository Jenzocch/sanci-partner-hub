...
### 53. 「請稍後再試」如果背後沒有任何一行 server log，等於把錯誤丟進黑洞——每個 generic 失敗都要帶一個短代碼，代碼同時出現在畫面和 log〔本專案 2026-09-06，owner：「客戶那邊出現問題時要有文字可以反映給我」〕
- **症狀**：`safeWrite` 明明回傳了 Postgres 的 `code`/`detail`，Server Action 卻只回 `PESAN.serverSibuk`（"Tidak bisa menyimpan sekarang. Coba lagi sebentar lagi."），而 `app/cabang/**` 裡**一個 `console.error` 都沒有**。店員看到的是一句無法轉述的話，owner 看 Vercel log 也是空的——兩邊都不知道發生了什麼，問題只能等它自己再發生一次。這跟 #10「DB 錯誤不得偽裝成業務結論」是同一家族：不是偽裝，是**蒸發**。
- **修法（`lib/safe-write.ts`）**：`catatGagal(aksi, detail)` 產生 `SP-XXXXX`（5 碼，去掉 0/O/1/I 免得口述抄錯），`console.error` 一行 JSON（`laporan`、`aksi`、code/detail、ISO 時間），回傳代碼；`pesan().serverSibukKode(kode)` 把「Kode laporan: SP-…, sebutkan ke SANCI」接在原句後面（`common.netReportCode`，三語）。店員只要轉述 7 個字元，owner 在 log 搜同一串就看到原因。**Postgres 原文仍然不上畫面**——檔頭那條規則沒有放鬆。
- **要記得的邊界**：像 `verifyActiveStaffInBranch` 這種只回 `"error"` 字串的共用函式，呼叫端拿不到 detail——讓函式自己 `console.error` 一行（帶 staffId/branchId/時間），呼叫端再產代碼；兩行 log 相鄰、靠 id 對得起來，不必為了傳 detail 改共用簽名（admin 端也在用）。
- **同一天的另一半：「按了沒反應」多半是欄位錯誤顯示在畫面外**。表單的 `errs[field]` 只渲染在該欄位下方，但手機上送出鈕在最底、空著的 Sales/Jalur/Package 早就捲到上面去了——server 明明有回「Sales wajib dipilih」，店員看到的是「什麼都沒發生」。修法：收到 field error 時 `scrollIntoView`＋`focus` 到那個 `[name=field]`，並在按鈕旁再渲染一次摘要（`cabang.formFieldSummary`）；`_form`／網路／partial 這些**頂部** banner 也要把卡片捲回頂端。通用檢查：任何「錯誤訊息渲染位置」和「觸發它的按鈕位置」不在同一屏的表單，都有這個病。
- **驗證**：tsc/eslint/build 三關＋隔離跑 9 條斷言（2000 個代碼格式、log 的 kode == 回傳的 kode、三語 `serverSibukKode` 含代碼且不殘留 `{kode}`、舊 `serverSibuk` 不變）。
- **2026-09-07 補完 admin 端（13 個檔案 62 處），並在補的過程中抓到 `catatGagal` 自己的 bug**：`JSON.stringify` **會把 Error 的 `message` 整個丟掉**——`message` 是 Error 原型上的 non-enumerable 屬性，而 supabase-js 的 `PostgrestError` 是 Error 的子類。實測一個 RLS 錯誤只記下 `{"name","details","hint","code"}`，`"new row violates row-level security policy…"` 這句**最能說明原因的話，從一個專門為了說明原因而存在的 log 裡靜靜消失**。cabang 端當初逐欄寫 `{code, detail}` 剛好避開，admin 端改成傳整個物件才撞上——**寫 log 的程式碼本身也會安靜失敗**。修法：`catatGagal` 內建 `bisaDicatat()`，遇到 Error 逐欄複製（含巢狀在 safeWrite 結果裡的），不交給 stringify。
- **通用檢查**：任何 `JSON.stringify(錯誤物件)`／把 error 塞進結構化 log 的地方，先問「這是不是 Error 的子類」——是的話 `message` 必須自己撈出來。**驗的方法**：用 `typescript` 的 `transpileModule` 把真檔案轉成 JS 再跑斷言（stub 掉 `build-id` 之類相依），驗的是**要 commit 的原文**而不是手抄的複本（跟 #51 同一個習慣）。11 條斷言全過，含「message 有被記到」與「巢狀 Error 也展開」。第一次嘗試用 regex 剝 TS 型別，剝出語法錯誤的垃圾——別再那樣做。
- **順帶更正一個過期的基準數字**：`docs/HANDOFF.md` 要求 `/offline` 維持 `○ 1.01 kB`，但在 d2fe7e8（本次改動之前）開 worktree 實際 build 出來就已經是 **1.02 kB**，現在 1.03 kB；First Load JS 兩邊都是 104 kB，且 `"Kode laporan"` 這串**沒有**進到 `/offline` 的 chunk（#38 的界線仍守住）。**基準數字要附「量到的日期＋當時的 commit」**，否則下一個人會為了一個早就過期的門檻去追不存在的 regression。

### 54. 「這個畫面印了 X」不能只看 JSX——後面的 CSS 可以把整格 display:none 掉；審計要嘛真的渲染，要嘛把 CSS 也讀完再下結論〔本專案 2026-09-07，audit「分店端誤用 SANCI 資源」時自己踩到〕
- **症狀**：讀 `proposal-editorial-document.js` 看到封面 JSX 有 Showroom（PT. WAHANA ERA INOVASI）和 Kontak（WhatsApp 0851-3318-5168）兩格，就向 owner 報「分店做的 Proposal 印的是 SANCI 的電話」。寫列印 harness 才發現 `.module.css` **第 3 行**的改版覆寫 `.coverMetaGrid>div:not(:first-child){display:none}` 早就把那兩格藏掉——紙上只有 SANCI logo 和頁尾「SANCI Proposal」，沒有電話。報告過度陳述，而 owner 是根據那句話選方案的。
- **證據等級（低→高）**：grep 到字串 ＜ 讀到 JSX ＜ 讀完 JSX **加全部 CSS（含 module 裡後面的覆寫塊）** ＜ 真的渲染出來量。給 owner 的「有沒有顯示」結論至少要第三級；會讓 owner 做決定的，要第四級。Playwright 印 PDF 的 harness 幾分鐘就跑完——**先跑再報**。
- **實作**：分店 Proposal 封面加 `.coverStore`（Toko 標籤／partner logo，無則不印／partner · cabang／WhatsApp）。刻意放在 `.coverMetaGrid` **外面**當兄弟節點——放進去會被同一條 `:not(:first-child)` 藏掉（CSS 裡留了註解）。5 個案例（無 store／有 logo／無 logo／超長名稱／無電話無分店）都 1 頁、sheet 297.1mm，區塊 16–43mm 由封面照片區吸收。partner logo 載入失敗 `onError` 隱藏，客人文件不留破圖。
- **同一輪的一般性檢查（以後每個新出口都要過一遍）**：分店端每一個「代表店家對客人說話」的出口，都問「客人看到的是誰的名字、誰的聯絡方式」。本次 5 個出口：Fonnte 公司號（分店端連 Server Action 一起刪，UI 藏起來不算）、Proposal 封面（補店家區塊）、WhatsApp 訊息文字（帶 partner 名，乾淨）、`/lihat` 客戶查單頁（「toko tempat Anda memesan」，乾淨）、`/p` 公開產品頁（SANCI 品牌、無聯絡方式，owner 決定維持）。
