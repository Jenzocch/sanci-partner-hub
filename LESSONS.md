...
- **驗證**：tsc/eslint/build 三關＋隔離跑 9 條斷言（2000 個代碼格式、log 的 kode == 回傳的 kode、三語 `serverSibukKode` 含代碼且不殘留 `{kode}`、舊 `serverSibuk` 不變）。**還沒做**：admin 端（`app/admin/actions*.ts` 有 62 個 `serverSibuk`）沒套，owner 自己撞到時仍然沒代碼——下次順手。
