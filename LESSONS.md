...
- **教訓**：**叫 owner 重跑任何會寫資料的腳本之前，先列出它會寫哪些欄位/檔案，再查 audit_logs 確認這些目標自初次執行後有沒有被手動改過**——有就先擋下來想清楚。設計匯入腳本時，「безопасно重跑」的正確定義是「重跑不會蓋掉比它新的資料」，不是「重跑不會出錯」。這與 #7（成功訊息不是證據）同族：一個寫著「安全」的標籤，要先問它保證的到底是哪一種安全。
